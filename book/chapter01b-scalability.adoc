...
Um exemplo de contagem utilizando tal recurso do Java 8 pode ser encontrado em link:../projects/MapReduceApp.java[MapReduceApp.java]. A única linha necessária para contar o total de pessoas por sexo, em paralelo, está dentro do método `long contaPessoas(char sexo)`.
