...
de criar uma aplicação cliente/servidor utilizando as
...
ou seja requisições síncronas onde a apliação fica parada à espera
...
No primeiro caso, a forma mais fácil de resolver é matar os processos Java em execução
digitando no terminal `sudo killall -9 java` (em sistemas Linux).
