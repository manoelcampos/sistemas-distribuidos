...
A aplicação pode utilizar qualquer servidor XMPP. Por exemplo, podemos conectar em servidores gratuitos como https://jabber.at ou
...
Por exemplo, se você criar uma conta em https://jabber.at, será fornecido um login. É importante notar que este login não é seu email. Um login XMPP tem o formato de um email, mas não é um endereço de email de fato.
Considerando que você tenha criado uma conta no site acima com o nome de usuário `manoelcampos`, então seu login será `manoelcampos@jabber.at`. Este é o login que deve usar para logar na aplicação aqui fornecida.
A partir do login, a aplicação descobre qual o domínio ao qual os usuários deste servidor XMPP estão associados.
Neste exemplo, o domínio é `jabber.at`.
...
- http://professionalxmpp.com[Livro "Professional XMPP Programming with JavaScript and jQuery"]
