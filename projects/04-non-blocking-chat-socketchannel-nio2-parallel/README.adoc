...
de uma forma eficiente (diferente da link:01-blocking-chat-socket[versão bloqueante]).
...
o recurso utilizado criará, por padrão, no máximo 3 Threads (pois uma Thread é a principal da aplicação, automaticamente
criada pela JVM quando a aplicação é executada). Assim, se existirem 3000 requisições de clientes, estas são distribuídas uniformemente entre as 3 Threads, cada uma processando 1000 requisições.
