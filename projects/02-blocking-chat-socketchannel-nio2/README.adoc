...
interfaces `ServerSocketChannel` e `SocketChannel` da API https://docs.oracle.com/javase/8/docs/technotes/guides/io/[NIO.2 do Java 7],
mais atuais que as classes `ServerSocket` e `Socket` que vêm desde o Java 1.
...
A forma de implementação desta versão difere da link:../01-blocking-chat-socket[anterior] apenas pelo uso das novas interfaces
